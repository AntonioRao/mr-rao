...
| `mr_rao/posizioni.py` | **Dove** stava, nel testo di partenza, ogni cosa che il motore ha tolto. Il motore restituisce il testo redatto, non le posizioni: chi deve intervenire sull'originale — togliere i glifi da un PDF, rimettere i separatori dentro un nome di file — le ricava qui. Nessuna decisione su cosa sia un dato personale: solo l'allineamento fra il prima e il dopo. Nato dentro `redazione_pdf.py` ed estratto nella 1.29.0, quando è servito al secondo chiamante: due copie sarebbero due modi diversi di tagliare lo stesso dato |
...
| `mr_rao/redazione_pdf.py` | Redazione di un PDF **senza rasterizzarlo**: toglie i byte dei glifi dal flusso di contenuto e ci mette il segnaposto, con un font standard. Il documento che esce è ancora selezionabile e pesa uguale. Raggiungibile dall'interfaccia: pulsante «Vedi l'anteprima» sul risultato, anteprima prima/dopo affiancate, rettangolo verde su ogni segnaposto |
