...
| `mr_rao/posizioni.py` | **Where** each thing the engine removed sat in the original text. The engine returns redacted text, not positions: whoever has to act on the original — removing glyphs from a PDF, putting the separators back inside a filename — works them out here. No decision about what counts as personal data: only the alignment between before and after. Born inside `redazione_pdf.py` and extracted in 1.29.0, when a second caller needed it: two copies would be two different ways of cutting the same value |
