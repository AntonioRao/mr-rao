...
Lo stato delle voci vale alla **1.24.0**. La fonte di verità resta git e il
...
| ID | Item | Perché, e cosa costa | Stato |
|----|------|--------|--------|
| P8.1 | **Anteprima prima/dopo anche per i `.docx`** | Il PDF ce l'ha dalla 1.23.0, ed è ciò che permette a chi deposita un atto di **vedere** cosa è sparito prima di consegnarlo. Per Word non c'è, e il `.docx` è il formato in cui si scrivono le lettere: il pubblico di questo programma ci passa più tempo che sui PDF.<br><br>**La forma non può essere la stessa, e va detto invece di prometterla.** Un PDF ha pagine perché qualcuno le ha impaginate; un `.docx` non le ha finché un motore di impaginazione non gliele dà, e quel motore non c'è — né LibreOffice né Word possono essere dipendenze di un programma che si vanta di non chiamare niente. Quindi non pagine affiancate come immagini, ma **contenuto affiancato**: originale e redatto resi in HTML con `mammoth`, che è già nel pacchetto e già dichiarato in `requirements.txt`. Nessuna dipendenza nuova.<br><br>**Il vincolo che decide se serve o inganna**: il riquadro deve dire che è un'anteprima **del contenuto e non dell'impaginazione**, altrimenti chi guarda conclude che il documento consegnato sarà così, e non lo sarà | Da fare |
| P8.2 | **Età e sesso: quattro forme che non vengono rilevate** | `_RE_ETA` e `_RE_SESSO` compongono le maiuscole con gruppi `(?i:…)` messi a mano, e in tre punti il gruppo **non copre** il pezzo che conta:<br>• in `(?P<b>\d{1,3})\s+anni\s+(?i:d'\|di\s+)et[àa]` la parte `et[àa]` sta **fuori**, quindi `45 anni d'ETÀ` non si vede mentre `ETÀ 44` sì;<br>• in `_RE_SESSO` la sigla `[MF]` sta fuori dal gruppo, quindi `sesso: f` minuscolo non si vede;<br>• `d'` non ammette spazi dopo l'apostrofo: `d'anni 78` sì, `d' anni 78` no.<br><br>E una quarta, trovata provando il pacchetto MSIX: **`Eta': 45`, con l'apostrofo al posto dell'accento**, non viene rilevato. Non è un caso di laboratorio: è come si scrive «età» in un documento battuto senza accenti, e succede spesso.<br><br>**Nessun dato esce**: questi due riconoscitori non sostituiscono mai niente, per scelta. Il danno è che il rapporto **conta meno di quello che c'è** — e il rapporto è tutta la ragione per cui la funzione esiste, perché «lasciate in chiaro 3 età, apposta» vale solo se sono davvero tre.<br><br>**Il costo vero non è la regex**: toccare il motore obbliga a rigenerare il corpus di conformità e a riallineare il port, che il 2026-08-11 è appena arrivato a 141/141. Per questo non è entrata nella 1.24.0 — si fa insieme al prossimo giro sul port, non da sola | Da fare |
| P8.3 | **Il cancello blocca la build che lo farebbe passare** | `scripts/build_portable.bat` fa girare la qualità al passo 2 su 7, e uno dei controlli (`tests/test_installer.py`) guarda il pacchetto **già presente** in `dist/`. Un pacchetto rimasto a metà — una build interrotta, o un eseguibile ancora aperto durante la copia — rende quel controllo rosso, e quindi impedisce la build che lo rifarebbe da capo. È successo il 2026-08-11 e si è sbloccato solo cancellando la cartella a mano.<br><br>Il controllo **è giusto** e non va indebolito: dice una cosa vera, cioè che il pacchetto su disco è incompleto. Sbagliato è **quando** viene fatta la domanda. La cartella la cancella già la build stessa, al passo 4; una verifica sul pacchetto finito appartiene a dopo la costruzione, non a prima.<br><br>Mezz'ora. Il modo di sbagliarla: spostarlo dopo **senza** che possa far fallire la build, e ritrovarsi a spedire un pacchetto incompleto perché ormai il build ha già detto OK | Da fare |
