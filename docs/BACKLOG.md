...
Lo stato delle voci vale alla **1.26.0**. La fonte di verità resta git e il
...
| P3.9 | **Il link allo Store, e la sezione sull'avviso di Windows da riscrivere** | Due cose, e la seconda conta più della prima. Il link `https://apps.microsoft.com/detail/9N7SJ4W88KQC` è già definitivo — lo Store ID non cambia — ma per due giorni ha risposto **`410 Gone`**: verificato, non supposto. Pubblicarlo prima avrebbe voluto dire un pulsante morto proprio sulla strada della fiducia, quella che esiste per togliere l'avviso «editore sconosciuto»: chi ci clicca è la persona più diffidente, e avrebbe trovato un errore. È lo stesso modo di rompersi in silenzio da cui nasce la disciplina del nome fisso `MrRao-Portable.zip`.<br><br>**FATTA il 2026-08-11**, il giorno in cui la certificazione è passata e la scheda è andata online. Il link `https://apps.microsoft.com/detail/9N7SJ4W88KQC` è entrato nei **due README**, in due punti scelti ciascuno: la tabella delle tre confezioni, dove la riga dice cosa cambia scegliendo quella strada (aggiornamenti gestiti da Windows, nessun avviso perché il pacchetto lo firma lo Store), e l'apertura della sezione sull'avviso di Windows, che ora dichiara subito che quella sezione riguarda le **altre due** confezioni. La portable resta la strada principale e il suo pulsante non è stato toccato.<br><br>**Non è entrato** il collegamento `ms-windows-store://pdp/?productid=9N7SJ4W88KQC`: fuori da Windows non fa niente, e un README lo legge anche chi Windows non ce l'ha. Le coordinate complete stanno in [STORE.md](STORE.md) | **Fatta** (2026-08-11) |
...
| P3.15 | **Le pagine pubblicate invecchiavano in silenzio** | Trovato facendo P3.11. `check_docs.py` esiste per impedire ai documenti di invecchiare senza che nessuno se ne accorga, e guardava **solo i `.md`**. Le landing HTML no — e infatti `docs/landing/index.html` ha dichiarato **v1.7.2** mentre `APP_VERSION` era 1.11.0: venti release di scarto, e nessun controllo capace di vederlo | **DONE** (1.12) — ottava invariante, `landing_invecchiate()`. **Tre dettagli che non sono dettagli**: usa `git ls-files` e non un glob (le bozze `02-*`/`03-*` sono gitignorate, un glob le avrebbe incluse facendo fallire il gate su file che non fanno parte del progetto); **regex dedicata**, perché le landing scrivono il numero attaccato a una `v` in un distintivo (`· v1.7.2`) e mai nella forma discorsiva che cerca la regex dei `.md`, e la `v` è obbligatoria apposta — un `\d+\.\d+\.\d+` nudo avrebbe pescato `127.0.0.1`, che nelle landing compare cinque volte; **due guardie contro il controllo morto** (zero landing tracciate è un problema, e zero versioni trovate è un problema che **nomina la regex** da aggiornare). Visto fallire in quattro modi, mutando anche il controllo stesso. 18 test.<br><br>**Restava però un secondo modo di invecchiare, chiuso nella 1.24.0.** Quel controllo guarda i *numeri* dichiarati dalla pagina; non guardava se la pagina pubblicata venisse ancora dal suo sorgente. Le due landing hanno `<style>` e `<script>` scritti dentro, e una CSP che li ammette **per impronta**: basta uno spazio cambiato nel sorgente senza rilanciare `_rebuild.py` perché l'impronta in `_headers` non corrisponda più, e il sito vada online **bianco** — nessun errore, nessun log, e ce ne si accorge solo passandoci. Ora `tests/test_landing_pubblicata.py` **rilancia il rigeneratore** su una copia di lavoro e pretende che non cambi niente: copre in un colpo il sorgente modificato senza rigenerare, la pagina pubblicata ritoccata a mano e le impronte rimaste indietro. Ricalcolare gli hash dai file pubblicati direbbe «tutto a posto» in tutti e tre i casi, perché quei file sarebbero coerenti con sé stessi: sarebbe un controllo che non può fallire |
...
### P3.6: la decisione, e perché è durata mezza giornata

**Questa sezione è la fotografia di una decisione poi ribaltata, e resta
scritta perché i suoi vincoli valgono ancora.**

Il 2026-08-09 la voce era stata approvata — *«si fa»*, con la forma descritta
qui sotto. **Lo stesso giorno la misura l'ha chiusa**: dentro il cancello
difendibile il guadagno è zero su 4,5 M caratteri, con due modelli diversi.
Il verdetto sta nella scheda di P3.6 in tabella, ed è quello che vale.

Quello che invece **non è stato ribaltato** è la forma obbligatoria qui sotto:
è il vincolo che ha permesso di misurare, ed è la ragione per cui il risultato
si può credere. Vale per qualunque modello futuro, e va letto prima di
riaprire la voce.

Cosa non basta per riaprirla: un modello nuovo, una licenza più pulita, un
file più leggero. Serve un **guadagno misurato dentro il cancello**, con lo
stesso banco.

Segue la forma approvata quel giorno, ed è quella che il collaudo più sotto
continua a descrivere. Non è un dettaglio implementativo — è la ragione per
cui si potrebbe fare.
...
| ID | Item | Stato |
|----|------|--------|
| P4.1 | Rinominare cartella repo `markitdown-webapp` → `mr-rao` | **FATTO** (2026-08-13). Dentro il repository non c'era **nessun** percorso assoluto: script, installer e CI hanno retto senza una modifica. Si è rotto solo il **venv**, che incide il percorso in 6.457 file e va rifatto da capo — e rifacendolo sei dipendenze sono salite di versione (in `requirements.txt` ci sono minimi `>=`, non versioni fissate), quindi `THIRD_PARTY.md` è stato rigenerato. Resta da rifare a mano il collegamento sul desktop |
...
| P5.6 | **Estensione browser che redige l'incolla verso le chat AI** | L'idea più preziosa e quella da non fare adesso. È l'unica che attacca il momento vero del rischio: tutto il resto dipende dal fatto che qualcuno decida di usare Mr. Rao **prima**.<br><br>**Perché no, in concreto.** (a) **Rompe la promessa centrale**: per redigere ciò che si incolla servono i permessi di lettura su `chat.openai.com` e `claude.ai`, cioè un'estensione che può leggere *tutte* le conversazioni. «Offline, non esce niente» diventa «fidati che non lo mando» — risolvibile (motore dentro l'estensione, nessun permesso di rete, manifest ispezionabile) ma la **spiegazione** diventa difficile, e il patrimonio di questo prodotto è che la spiegazione è facile. (b) **Il motore è Python**: 2355 test, i checksum, gli elenchi di cognomi nel browser non ci vanno. O si riscrive in JS — **due motori che divergono**, e in un motore di redazione la divergenza è una fuga silenziosa — o l'estensione parla con il Flask locale, e allora servono un server acceso, una porta e il CORS. (c) Revisione dello store con permessi larghi su siti AI, mentre Mr. Rao sta ancora combattendo una **diversa** battaglia di fiducia (SmartScreen, firma) non ancora vinta.<br><br>**Prima di questa va provata P5.3**, che vale l'80% a un decimo del prezzo. Se un giorno si fa, si comincia dal threat model, non dal codice | Non ora — decisione registrata |

## P6 — Dal confronto con lo stato dell'arte (2026-08-10)

**Da dove vengono queste voci.** Non da idee in astratto: dalla lettura del
codice, dei report tecnici e della documentazione degli strumenti che oggi
fanno un lavoro vicino al nostro — motori di anonimizzazione italiani a
riconoscimento automatico, servizi legali di redazione documentale,
estensioni che filtrano gli incolla verso le chat AI. Vale il criterio di
P5: ogni voce va pesata contro il rischio di allargare Mr. Rao fino a
fargli perdere il fuoco.

**La conferma che vale più delle voci.** Chi è partito dal riconoscimento
automatico, in produzione, **non usa il modello da solo**: lo affianca a una
rete di regole e cifre di controllo, e un checksum valido sovrascrive il
modello. È «il pattern propone, il validatore decide» arrivato da un'altra
strada. Il nostro motore **è** quella metà lì. Non siamo indietro sugli
identificatori con un conto dietro; siamo scoperti dove serve capire la
prosa (nomi in contesti insoliti, ragioni sociali).

**E una cosa che non ha nessuno degli altri**: le credenziali. Nelle
tassonomie che abbiamo guardato non c'è traccia di chiavi API, token o
password. Noi sì.
...
| ID | Item | Perché, e cosa costa | Stato |
|----|------|--------|--------|
| P6.1 | **Segnaposto numerati per valore distinto** (`{{NAME_1}}`, `{{NAME_2}}`) | Oggi tre persone diverse diventano tre `{{NAME}}` identici, e il documento redatto **perde il senso**: «`{{NAME}}` ha citato `{{NAME}}` davanti a `{{NAME}}`» non si legge, e un LLM non ci può ragionare. Chi lo fa numera per valore distinto, e lo stesso valore riceve sempre lo stesso numero.<br><br>**È la voce con il rapporto valore/costo migliore di tutto l'elenco**, e non richiede la reversibilità: la numerazione è una proprietà del testo redatto, il dizionario è un'altra cosa (vedi P6.9).<br><br>**Le tre condizioni che decidono se è una funzione o un difetto.** (a) **La numerazione non deve creare un canale**: due valori diversi devono avere numeri diversi *dentro un documento*, mai un numero stabile fra documenti — sarebbe un identificatore persistente, cioè un dato personale nuovo inventato da noi. (b) Va estesa a **tutti** i segnaposto o a nessuno: `{{NAME_1}}` accanto a `{{IBAN}}` è peggio di due schemi coerenti. (c) I test dei segnaposto non riassorbiti (`test_segnaposto_non_riassorbiti.py`) vanno rifatti sulla forma nuova, e la guardia contro il riassorbimento — il difetto `{{NINO}}` della 1.19.1 — va riprovata: `{{NAME_12}}` ha una cifra dentro, e le cifre attraggono i riconoscitori.<br><br>**FATTA nella 1.20.0, e accesa di default.** Le tre condizioni reggono tutte e tre (la mappa vive nel `RedactionReport` e muore con la conversione; tutti i 51 punti convertiti; la guardia estesa alle forme numerate, 127 casi). Quattro cose che la previsione non conteneva:<br>• **una regola aveva smesso di funzionare in silenzio**: i pattern che si agganciano a un segnaposto già inserito lo cercavano alla lettera (`\{\{EMAIL\}\}`), e con `{{EMAIL_1}}` «nome accanto all'indirizzo di posta» non scattava più — `Rao <a.rao@…>` usciva con `Rao` in chiaro. Ora c'è `_rif_segnaposto` in un posto solo;<br>• **ha scoperto un difetto vecchio**: `_RE_SECRET_KV` si rimangiava il proprio segnaposto come valore, e il rapporto contava `secrets: 3` per **un** segreto. I tre `{{SECRET}}` identici lo nascondevano;<br>• **serve una rinumerazione finale**: i numeri seguivano l'ordine dei riconoscitori, non del testo (`{{SECRET_2}} = {{SECRET_1}}`). E marcata con una sentinella, o rinumererebbe anche i segnaposto che stavano già nel documento in ingresso;<br>• **la domanda 8 delle FAQ è stata riscritta**: costruiva su «non sono numerati» la conclusione che in uscita non si potesse ricollegare chi era chi. La numerazione **toglie quella proprietà** dentro un documento (i numeri dicono quante persone distinte ci sono e dove compare ciascuna), e lascia intatte le altre due — nessuna mappa scritta, nessun numero stabile fra documenti | **Fatta** (1.20.0) |
| P6.2 | **Rilevato ma non sostituito** | Oggi spegnere un riconoscitore vuol dire **non cercarlo**. Altrove le due cose sono separate: una categoria deselezionata viene comunque rilevata, e semplicemente non sostituita. Per chi deve far confrontare degli importi a un LLM, o tenere età e sesso in una cartella clinica, la differenza è tutta.<br><br>**Il valore vero è nel rapporto**: «ho lasciato in chiaro 3 importi, apposta» è informazione per un DPO; il silenzio non lo è. Oggi un riconoscitore spento non lascia traccia, e chi rilegge il documento non ha modo di sapere se lì dentro non c'era niente o se abbiamo guardato dall'altra parte.<br><br>Costo modesto: `RedactionReport` ha già i conteggi, serve un terzo stato nelle opzioni (`sostituisci` / `segnala` / `ignora`) e una colonna nel rapporto. Il rischio è di interfaccia: tre stati per dieci riconoscitori sono trenta interruttori, e vanno raccontati bene o diventano rumore.<br><br>**FATTA nella 1.20.0.** Non come tipo nuovo degli interruttori ma come **elenco di categorie** (`segnala=("iban", "amounts")`): niente si rompe, e le due domande restano separate — l'interruttore dice *se guardare* (per famiglia: `fiscal` copre CF, P.IVA, IBAN e carte), `segnala` dice *cosa fare di ciò che si trova* (per categoria), che è la finezza che serve davvero. Il rischio d'interfaccia era reale: le 24 caselle stanno in una sezione richiudibile, chiusa di default.<br><br>Tre cose emerse strada facendo:<br>• **il frontmatter taceva**, e il frontmatter è l'unica parte del rapporto che viaggia col documento — chi lo riceve fra sei mesi non ha la richiesta HTTP. Ora c'è un blocco `detected_not_replaced:`, separato da `redactions:`;<br>• **i codici recuperati dall'OCR sarebbero usciti riscritti**: tre chiamanti passano al motore la versione *corretta* del valore, e restituirla in modalità «segnala» avrebbe cambiato il documento senza sostituire niente. Risolto con un parametro `originale`;<br>• **la guardia del vocabolario era cieca quanto l'elenco che sorvegliava**: l'estrazione dal sorgente leggeva solo le chiamate dirette e saltava le sei che passano da `_replace_all`. Mancavano `emails` e `termini`, e il test era verde | **Fatta** (1.20.0) |
| P6.3 | **Tabella ISO 13616 delle lunghezze IBAN per paese** | `_RE_IBAN_SPAZIATO` oggi tira a indovinare dove finisce un IBAN scritto a gruppi: `(?:[ -][A-Z0-9]{2,6}){2,9}`, e a scartare i candidati sbagliati ci pensa il mod-97. Funziona, ma indovinare il confine ha due modi di sbagliare: **inghiottire le parole vicine**, o fermarsi a metà **lasciando la coda dell'IBAN in chiaro** — e il secondo è una fuga silenziosa, perché il rapporto dice «1 IBAN sostituito».<br><br>La tabella delle lunghezze per paese (`IT` 27, `DE` 22, …) serve proprio a sapere dove tagliare. Mezza giornata, dati pubblici, e si misura: il corpus deve restare verde e i casi nuovi vanno costruiti sugli IBAN spaziati di paesi con lunghezze diverse.<br><br>**FATTA nella 1.20.0, ma la voce era sbagliata su due punti**, e vale la pena scriverlo perché è il genere di errore che questo elenco fa spesso:<br>• **la tabella ISO 13616 c'era già** (`_IBAN_LUNGHEZZE`, usata da `iban_checksum_ok`). Non serviva prenderla da fuori: serviva **usarla anche per tagliare**, non solo per validare;<br>• **il difetto vero era un terzo, non previsto qui, e peggiore**. Non un troncamento ma un **silenzio**: gli IBAN la cui lunghezza dà resto 1 finiscono con un gruppo di **un** carattere (`PT92 … 6LGU A`), il pattern pretendeva gruppi da almeno due, e non riconosceva **niente** — zero IBAN dichiarati su un documento che ne conteneva uno. Colpiva Portogallo, Svizzera, Croazia, Brasile, Ucraina, Qatar, Palestina, São Tomé.<br><br>Ora il pattern è goloso apposta e `_prefisso_a_norma` taglia alla lunghezza di legge: quel che prende in più torna al testo. **79 Paesi su 79, 5 semi ciascuno** (`tests/test_iban_lunghezza_di_legge.py`) | **Fatta** (1.20.0) |
| P6.4 | **Quattro categorie italiane che non abbiamo**: `TARGA`, `ORG` (ragione sociale), `CATASTO` (foglio/particella/subalterno), `DOCID` (n. R.G., protocollo, repertorio) | Sono le categorie per cui esistono i motori italiani specializzati, e coprono il pubblico che a noi interessa: notai, avvocati, commercialisti.<br><br>**Ma qui non c'è solo un buco: c'è una divergenza da decidere, e va decisa prima di scrivere codice.** «targa», «protocollo» e «repertorio» da noi stanno nel vocabolario delle parole che dicono di **non** redigere — è ciò che impedisce a ogni numero di pratica di essere letto come un telefono. Altrove li redigono, perché in un atto il numero di ruolo identifica le parti. **Hanno ragione tutti e due, per clienti diversi**: per un notaio il riferimento catastale *è* il dato più sensibile della frase; per un'azienda il numero di protocollo è ciò che permette di ritrovare la pratica, e toglierlo rende il documento inservibile.<br><br>Quindi: non un interruttore generale ma **un pacchetto «atti e pratiche»**, spento di serie, sulla falsariga dei pacchetti nazionali già esistenti. `CATASTO` è il candidato migliore per cominciare — ha parole di contesto fortissime («foglio», «particella», «sub.») e quindi pochi falsi positivi. `ORG` è il peggiore: lo dichiarano loro stessi come la classe dove il modello sbaglia di più, ed è una classe aperta che con le sole sigle (`S.r.l.`, `S.p.A.`) prende poco e con i vocabolari prende troppo.<br><br>**FATTA per tre categorie su quattro. `ORG` resta fuori, ed è una decisione, non un rinvio.**<br><br>Il pacchetto `ATTI` esiste ed è spento di serie, con **due assi** come i pacchetti nazionali: l'interruttore `atti` dice *quale dato*, il pacchetto dice *per quale mestiere*. C'è un test per ciascuno dei due versi, perché un interruttore che non può spegnere niente è decorativo.<br><br>• `CATASTO` — foglio **e** particella insieme, subalterno facoltativo. Il foglio da solo è la pagina tre di una relazione;<br>• `PRATICA` (era `DOCID`) — R.G., protocollo, repertorio, raccolta, cronologico. **Sparisce il numero e resta l'etichetta**, come per gli appellativi: «Prot. n. `{{PRATICA}}`» si legge ancora. Servono due cifre o l'anno accanto, ed è l'unica parte della regola che sappia dire di no — «Protocollo n. 5» di una convenzione non è una pratica;<br>• `TARGA` — `AB 123 CD`, tutto maiuscolo **oppure** tutto minuscolo, mai misto: il maiuscolo secco perdeva 135 targhe scritte `vm916jx` su un corpus di atti, e il misto è la firma di un frammento di OCR, non di una targa. Il pattern propone, **l'alfabeto decide**: I, O, Q e U sulle targhe non esistono. La forma da ciclomotore (`AB 12345`) è troppo comune per stare in piedi da sola e pretende la parola davanti.<br><br>**Il prezzo, misurato e non stimato.** Col pacchetto spento — cioè di serie — il corpus pubblico non si muove di un'unità (moduli in bianco 96 e 22, prosa vera 893) e nessuno dei 239 casi di conformità cambia. Col pacchetto **acceso**: 90 sostituzioni sulle Gazzette e 1 sui moduli italiani, e sono state **guardate una per una** — `prot. n. 26597`, `rep. n. 8757`, `protocollo n. 61238`. Sono tutte e 91 numeri di pratica veri: zero falsi positivi. Zero targhe su 47 documenti, il che dice che il pattern non spara ma non lo mette alla prova: quello lo fanno i test.<br><br>**Perché `ORG` non si fa.** Le ragioni scritte sopra non sono cambiate, e ora ce n'è una in più che viene dalla misura: le tre categorie fatte hanno pagato zero perché ognuna ha **qualcosa che sa dire di no** — una parola di contesto obbligatoria, una coppia richiesta, un alfabeto chiuso. `ORG` non ha niente del genere. Con le sole sigle prende `Rossi S.r.l.` e perde `Banca Intesa`; con un vocabolario di ragioni sociali prende ogni maiuscola del documento. E il costo cadrebbe dove sbagliare si vede di più, perché il nome di un'azienda in un atto **è il contesto**: toglierlo lascia una frase che non dice più di che pratica si parla.<br><br>Riaprirla vuol dire portare un modo di dire di no che oggi non abbiamo — non un elenco più lungo | **Fatta** (1.23.0); `ORG` **non si fa** |
| P6.5 | **Stringhe ad alta entropia come credenziale** | Gli strumenti che filtrano gli incolla verso le chat AI le elencano accanto alle forme note. Oggi `_scrub_secrets` copre le forme fisse (PEM, AWS, GitHub, JWT, Bearer) più il ramo debole etichetta/valore. Un token di formato ignoto senza etichetta davanti passa.<br><br>**È la voce più facile da sbagliare di tutto l'elenco**, e in una direzione sola: l'entropia alta ce l'hanno anche gli hash dei commit, gli UUID, i nonce, le firme base64 dentro un documento tecnico, e i codici a barre. Una soglia tarata male trasforma ogni documento tecnico in un colabrodo di `{{SECRET}}`, e questo prodotto ha già imparato che uno strumento che cancella mezzo documento viene disattivato.<br><br>**La forma che la rende sicura**: entropia **più** contesto, non entropia da sola — cioè lo stesso mestiere del ramo debole che c'è già. E la soglia si tara su un corpus a verità zero fatto di **documenti tecnici**, non di prosa: il numero non si sceglie a occhio, si misura muovendolo in peggio e guardando in che verso si muove il conto dei falsi positivi.<br><br>**Riscontro sul campo (20/08/2026)**, leggendo il pacchetto pubblicato dell'estensione concorrente PasteSecure: la implementa, e nella forma che questa voce prescriveva gia' — non entropia nuda, ma `shannonEntropy` **dentro** una funzione di giudizio (`looksLikeHighEntropySecret`) che pesa anche lunghezza, presenza di cifre e alternanza di maiuscole. Non e' un'idea nuova per noi: e' la conferma che «entropia piu' contesto» regge in produzione e non solo sulla carta. Da li' non si copia niente — serve a sapere che la strada gia' scelta e' percorribile | Da valutare |
| P6.6 | **Gli enti pubblici non sono dati personali** — verificare che non li stiamo togliendo | Le tassonomie italiane serie escludono i tribunali di proposito: «Tribunale di Roma» è un ente pubblico, resta nel testo come contesto. Stessa scelta per gli appellativi (`Dott.`, `Avv.`), che sono qualifiche e non identificatori.<br><br>Da noi non è mai stato deciso esplicitamente, e va **misurato prima di dichiararlo**: un documento con «Tribunale di Milano», «Comune di Torino», «Agenzia delle Entrate» passato al motore, e si guarda cosa esce. Se li togliamo è un falso positivo su una classe intera, ed è il tipo di falso positivo che un avvocato nota alla prima riga.<br><br>Mezza giornata, quasi tutta di misura.<br><br>**FATTA nella 1.20.0, e la misura serviva: era mezza rotta.** Gli enti nudi erano già a posto — `Tribunale di Roma`, `Comune di Torino`, `Agenzia delle Entrate` restano interi, e su `Dott. Bianchi` togliamo il cognome lasciando l'appellativo — un titolo è una qualifica, non un identificatore. La trappola era un'altra: **gli enti intitolati a una persona**. `_ENTITY_WORDS` conteneva `ospedale` e `istituto`, non `policlinico` né `teatro`, quindi `Policlinico Agostino Gemelli` diventava `{{NAME}}` — la frase perdeva il soggetto.<br><br>Aggiunte ~45 parole (edifici e istituzioni). **Il prezzo, misurato e non stimato**: una parola d'ente scherma l'intera sequenza maiuscola *adiacente*, quindi `Clinica Mario Rossi` ora è schermato per intero — come lo erano già `Ufficio Mario Rossi` e `Fondazione Mario Rossi`. Non vale per le forme con punteggiatura o ruolo in mezzo (`Policlinico Gemelli - referente Dott. Mario Rossi` resta protetto).<br><br>Il test ha un elenco di parole **tenute fuori apposta** (`studio`, `notaio`, `perito`): è l'unica parte che si accorge di un'aggiunta sbagliata, ed è stata verificata al contrario prima di scriverla | **Fatta** (1.20.0) |
| P6.7 | **Glossario e FAQ nella documentazione** | I servizi legali di redazione documentale pubblicano glossari da un centinaio di definizioni e oltre 140 risposte. Per il pubblico di Mr. Rao — DPO, studi, uffici pubblici — un glossario che distingua **pseudonimizzazione e anonimizzazione**, titolare e responsabile, dato personale e dato identificativo, vale più di una funzione: è ciò che rende il documento citabile in una valutazione d'impatto.<br><br>Costo basso, e va sotto `check_docs.py` come tutto il resto o invecchia in silenzio | Da valutare |
| P6.8 | **Una pagina su cosa la redazione NON garantisce** | Altrove esiste una sezione di casi di studio sulla de-anonimizzazione; noi abbiamo la cosa più difficile da imitare — `docs/VERIFICARE.md` in Mr. Rao Plus, che accanto a ogni promessa mette il modo di controllarla — ma **non abbiamo una pagina che spieghi i limiti del risultato**.<br><br>Il contenuto c'è già sparso: che togliere gli identificatori non rende un testo anonimo (l'inferenza dal contesto resta), che un dizionario reversibile **è** un archivio di dati personali, che i sospetti sono segnalati e **non tolti**. Metterlo in una pagina sola è la cosa più coerente con la voce di questo progetto, ed è precisamente quello che nella documentazione dei concorrenti non c'è | Da valutare |
| P6.9 | **Sostituzione reversibile con dizionario locale** | Gli altri la mettono al centro: si redige, si manda il testo con i segnaposto, si ripristinano i valori veri nella risposta. Cambia la natura dello strumento — da «prepara un documento da archiviare» a «usa l'LLM sui documenti veri».<br><br>**Non è in cima e c'è un motivo.** Il dizionario placeholder → valore **è un archivio di dati personali**, con la particolarità di essere l'unico file del sistema in cui i dati stanno tutti insieme, scremati dal contesto e pronti da leggere. Oggi Mr. Rao non ha niente da proteggere perché non conserva niente; questa funzione crea la cosa da proteggere. Chi la offre lo sa, e infatti la rende disattivabile.<br><br>Va aperta solo con una risposta scritta a: dove sta il file, per quanto tempo, chi lo cancella, e cosa succede se qualcuno se lo porta via. **P6.1 va fatta prima e da sola**: la numerazione dà buona parte del beneficio senza creare l'archivio.<br><br>**CHIUSA il 2026-08-10: non si fa.** Decisione dell'autore, con tre ragioni.<br>• **Crea la cosa da proteggere.** Oggi Mr. Rao non conserva niente, ed è il motivo per cui non ha niente da farsi rubare. Quel dizionario sarebbe l'unico file in cui i dati personali stanno tutti insieme, scremati dal contesto e pronti da leggere — la forma peggiore in cui possano stare;<br>• **il beneficio grosso è già stato preso.** La ragione principale per cui la si vuole è che il testo redatto resti leggibile e ragionabile, e quello lo dà P6.1, che è fatta e non crea nessun archivio;<br>• **il flusso non è quello di Mr. Rao.** Rimettere i valori richiede che la risposta dell'AI rientri nello strumento. Mr. Rao è un convertitore: prendi un file, ottieni un markdown. Il posto dove quel flusso vivrebbe è l'estensione, e lì sta già nel «non fare» con la stessa ragione.<br><br>Riaprirla richiede di rispondere per iscritto alle quattro domande qui sopra, non di cambiare idea | **Non si fa** — decisione registrata |
| P6.10 | **Età e sesso**: riconosciuti e messi nel rapporto, mai tolti | Sono **quasi-identificatori**, ed è una categoria diversa da tutto il resto del motore: «45 anni» da solo non identifica nessuno, ma insieme a un comune piccolo e a una professione sì — ed è esattamente così che si de-anonimizza un archivio.<br><br>**FATTA nella 1.23.0, e senza segnaposto.** Non c'è nessun percorso che li sostituisca, e la ragione è che le tre alternative non sono equivalenti: toglierli non protegge nessuno di più e rende il documento inservibile per l'unico uso per cui era stato preparato (cartella clinica, statistica del personale, perizia: chi le passa al motore sta chiedendo proprio quei due dati); lasciarli in silenzio vuol dire che chi rilegge non sa che ci sono. Restano nel testo e **compaiono nel rapporto**, nel blocco `detected_not_replaced` che è separato dalle sostituzioni.<br><br>Si riconoscono solo dove **il contesto è una dichiarazione** — `di anni 45`, `45 anni di età`, `età: 45`, `45enne`, `sesso: F`, `genere femminile`. Il `45 anni` nudo non si guarda: è quasi sempre una durata («dopo 45 anni di servizio»), ed è una rinuncia dichiarata, non un difetto.<br><br>Due cose tengono ferma la decisione: il riconoscitore **restituisce il testo identico** (c'è un test che lo verifica e uno che cerca `{{ETA}}` nel sorgente), e `eta`/`genere` **non stanno in `CATEGORIE`** — così nel pannello «segnala anziché sostituisci» non compare una casella che non sarebbe attaccata a niente. Chi li vuole togliere davvero ha già l'elenco «nascondi sempre», ed è questo a rendere onesta la scelta | **Fatta** (1.23.0) |
| P6.11 | **`TIME` (orari) come categoria a sé** | Le tassonomie dei motori a riconoscimento automatico la elencano accanto a `DATE`, e la domanda è venuta guardando la loro mappatura sulle nostre categorie.<br><br>**Non si fa, e la ragione è che non è un dato personale.** «Alle 14:30» non identifica nessuno e non lo aiuta a fare: è la componente meno discriminante di qualunque timestamp, quella che in un quasi-identificatore aggiunge la frazione di informazione più piccola. Quello che identifica è la **data**, che trattiamo già — e la trattiamo con l'interruttore spento di serie e legata al contesto di nascita, che è la parte davvero sensibile.<br><br>La seconda ragione è il prezzo: un orario ha la forma `\d{1,2}:\d{2}`, cioè la forma di una durata, di un punteggio, di un riferimento a un comma, e di metà delle celle di un foglio di calcolo. È una categoria che paga tantissimo e rende quasi niente — l'opposto delle tre di P6.4, che sono passate perché ognuna aveva **qualcosa che sapeva dire di no**.<br><br>La terza è che nei verbali togliere l'ora rompe il documento: «alle 14:30 il verbalizzante dà atto» senza l'ora non è più un verbale.<br><br>Riaprirla vuol dire portare un caso d'uso in cui l'orario **da solo** identifica qualcuno, non un elenco di chi altro lo fa | **Non si fa** — decisione registrata |
| P6.12 | **Redigere un PDF restituendo un PDF**, non un'immagine | Oggi un PDF entra e ne esce del Markdown. Chi deve archiviare o depositare l'atto vuole indietro **il documento**, e la strada ovvia — rasterizzare le pagine e disegnarci sopra dei rettangoli — è la peggiore di tutte: pesa 22 volte tanto, rende il documento inutilizzabile, e i rettangoli neri **si tolgono in un minuto** perché il testo resta sotto.<br><br>**Il motore è fatto** (`mr_rao/redazione_pdf.py`, 17 test in `tests/test_redazione_pdf.py` più 5 in `tests/test_redazione_pdf_fuori_dal_flusso.py`). Si tolgono i byte dei glifi dal flusso di contenuto e ci si mette il segnaposto con un font standard: il PDF che esce è ancora selezionabile e ricercabile, pesa uguale, e **il dato non è nel file** — non è coperto, è tolto.<br><br>**Misurato su 61 documenti veri, 1400 pagine**: 985 valori da togliere, **985 tolti**, 3 pagine su 1400 non trattate, peso medio **0,94x**, 50 secondi. La verifica porta un numero che il modulo non calcola (il totale dichiarato dal motore), altrimenti userebbe la stessa funzione con cui taglia e uscirebbe verde senza guardare niente.<br><br>**Le trappole che hanno prodotto quei numeri**, tutte da documenti veri e nessuna immaginabile a tavolino:<br>• l'API a oggetti del motore PDF **non basta** — il testo di una Gazzetta sta dentro un Form XObject e da lì gli oggetti non si rimuovono; in una presentazione ogni oggetto è una parola sola e «Mario Rossi» non è riconoscibile; in un manuale gli oggetti restituiscono stringa vuota. Zero oggetti operabili su tre documenti, e 73 sostituzioni su 73 sopravvissute;<br>• **il «cosa» e il «dove» vanno a fonti diverse**: cosa togliere lo decide il testo estratto (quello su cui il motore ha i suoi test), la mappa dei glifi dice solo dove cercarlo. Questa separazione da sola ha portato le sopravvissute da 11 a 0 sul primo documento;<br>• **i valori non si ricavano con un diff**: `CAFIERO` contro `{{NAME_1}}` condivide la «A» e la «E», e l'allineamento restituiva tre frammenti — «C», «FI», «RO» — troppo corti per essere cercati. Il cognome restava intero **e nessun conteggio se ne accorgeva**. Ora si legge la struttura: i pezzi fra i segnaposto sono copie letterali e fanno da ancora;<br>• lo spazio fra due parole spesso **non è un carattere** ma un arretramento nell'array, e l'a capo va messo solo se cambia davvero la riga — con un a capo a ogni spostamento «Il Ministro:» e il cognome arrivavano separati.<br><br>**E l'interfaccia, che era la metà mancante** — una funzione che dalla GUI non esiste, per chi usa il programma non esiste. Il pulsante compare **solo** quando il documento di partenza è un PDF (un comando che si può premere e non può funzionare è peggio di un comando assente) e apre un pannello con l'**anteprima prima/dopo della stessa pagina, affiancate**. Affiancate e non alternate: la domanda di chi guarda non è «com'è adesso» ma «cosa è cambiato», e quella risposta si legge solo con le due davanti insieme. Sotto i 900px vanno una sopra l'altra.<br><br>Si rendono **immagini** invece di mostrare il PDF in un riquadro: è l'unica cosa che funziona uguale nella finestra dell'applicazione, in Edge e in Firefox, senza dipendere da un lettore PDF incorporato che c'è o non c'è.<br><br>Tre cose che il pannello dice e che non sono decorazione: quante sostituzioni, **quali pagine non sono state trattate** (sempre, anche quando sono zero, nella tinta dei sospetti — che qui vuol dire «tocca a te guardare»), e il **rifiuto esplicito delle scansioni**, perché disegnarci sopra dei rettangoli sembrerebbe una redazione e non lo sarebbe.<br><br>Il PDF di partenza resta **nella memoria del browser** e le due chiamate lo rispediscono: il server non conserva niente fra l'una e l'altra, ed è la ragione per cui Mr. Rao non ha niente da farsi rubare. Le opzioni della pagina viaggiano con la richiesta, altrimenti il file scaricato sarebbe redatto in modo diverso da quello appena visto sullo schermo — e niente lo direbbe.<br><br>Verificato con misure sulla pagina vera, non a occhio: pagine affiancate della stessa larghezza, nessuno scorrimento orizzontale né a 1280 né a 375, contrasti fra 8,3:1 e 17,7:1 dove AA chiede 4,5.<br><br>Restano fuori, dichiarati: le **scansioni** (nessun testo estraibile, nessun glifo da togliere — il modulo lo dice e non tocca niente) e gli operatori `'` e `"`.<br><br>**Il testo dentro annotazioni e campi modulo è entrato nella 1.24.0** (`_redigi_annotazioni`), ed era il limite peggiore dei tre: quel testo non sta nel flusso della pagina, quindi usciva **intero** da un file chiamato `-redatto.pdf` — un codice fiscale leggibile dentro un documento il cui nome dice il contrario. Insieme al valore si butta l'aspetto memorizzato del campo (`/AP`) e si accende `NeedAppearances`: senza, sullo schermo resterebbe disegnato il nome di prima, con il dato tolto solo sotto — cioè la stessa illusione dei rettangoli neri, un piano più giù.<br><br>**E il rifiuto delle scansioni è diventato per pagina, non per documento.** Guardava se l'*intero* documento fosse senza testo, quindi un allegato firmato a mano in mezzo a pagine digitali usciva contato fra quelle trattate. Una pagina **bianca** invece non è un allarme e resta silenziosa: le due si distinguono dalla presenza di un'immagine, che è il motivo per cui il testo non c'è (`_pagina_e_una_scansione`) | **Fatta** (1.23.0) |

### Corpora etichettati di terzi: cosa si puo' usare (2026-08-10)

Sono i corpora etichettati su cui si addestrano i motori a riconoscimento
automatico. **A noi non servono per addestrare** -- il nostro motore non si
addestra -- ma servono per
...
sono nomi italiani. E' lo stesso limite gia' dichiarato per
`DeepMount00/pii-masking-ita` -- valori Faker anglosassoni in contesto
italiano -- e vale anche qui.
...
| ID | Item | Perché | Stato |
|----|------|--------|--------|
| P7.1 | **Il banco sul corpus pubblico non può girare, e nel mentre nasconde l'altra metà** | Lo dice il repository stesso ([`scripts/scarica_corpus_pubblico.py`](../scripts/scarica_corpus_pubblico.py)): il corpus originale (54 documenti) **non è recuperabile**, quello ricostruibile ne ha 47, e «i numeri attesi vanno rifatti su di lui». Non sono mai stati rifatti. Quindi il test è in una posizione da cui non esce: **senza** corpus salta, **con** il corpus ricostruibile fallisce sull'impronta. Non esiste una configurazione in cui misuri qualcosa.<br><br>**E c'è un secondo difetto sotto il primo.** Il controllo sull'impronta fa `return` (`bench_corpus_pubblico.py`, in `confronta`): non è un avviso premesso agli altri controlli, li **salta tutti**. La metà a verità zero — i moduli in bianco, dove ogni sostituzione è un errore — non viene mai valutata quando l'impronta non torna. Sono 226 sostituzioni mai guardate, ed è così che è emersa P7.2.<br><br>**Cosa fare, nell'ordine**: (a) l'impronta diversa deve **avvisare e proseguire**, non uscire — i gruppi a verità zero non dipendono dal corpus congelato, si possono sempre valutare; (b) ricongelare l'atteso sul corpus riproducibile (verificato intero, 47/47) **dopo** aver chiuso P7.2, altrimenti si congela il difetto insieme al riferimento <br><br>**FATTA (1.21.0), e la seconda metà era la più importante.** L'impronta diversa ora **avvisa e prosegue**: i moduli in bianco non dipendono dall'atteso congelato, e saltarli voleva dire spegnere l'unico controllo che l'impronta non toccava. Guardando le 226 sono usciti **tre difetti veri** (vedi P7.2), non uno.<br><br>**E la soglia era sbagliata in partenza**: «zero» è falso su qualunque modulo ufficiale, perché porta i recapiti dell'ente che lo pubblica e a volte la firma di una persona vera — `Rossella Orlandi` su un provvedimento dell'Agenzia non è un falso positivo, è un'etichetta sbagliata del corpus. Ora è un **ratchet per categoria**: crescere è un guasto, calare passa. Congelato sul corpus riproducibile (47/47 verificati) **dopo** aver chiuso P7.2, non prima | **Fatta** (1.21.0) |
| P7.2 | **`on` era nell'elenco dei titoli, e il punto era facoltativo** | `reported on Form 1125-A` usciva `on {{NAME_1}} 1125-A`; `included on Schedule K` perdeva la parola `Schedule`.<br><br>**La prima diagnosi era sbagliata, e vale scriverlo.** Era stata attribuita al vocabolario degli enti, «solo italiano, mancano gli equivalenti inglesi». Falso: `Income included on Quadro K` sbagliava identico **in italiano**. Il grilletto era la parola `on` — abbreviazione di «onorevole» — dentro `_TITLES`, con `\.?` che rendeva il punto facoltativo. Ogni `on` seguito da una parola maiuscola diventava una persona. Non un difetto dei documenti inglesi: un difetto che i documenti inglesi **rivelano**.<br><br>Corretto con una classe di abbreviazioni che pretendono il punto (`On. Mario Rossi` funziona, `on Form` no). **Nomi inventati sui moduli IRS: da 100 a 2.**<br><br>Guardando il resto ne sono usciti altri due, entrambi rivelati dallo stesso corpus:<br>• **`at` nudo letto come chiocciola**: `available at IRS.gov`, `visit us at IRS.gov` finivano in `{{EMAIL}}` — dieci falsi su undici. Ora un `at` senza parentesi pretende che **anche il punto sia offuscato** (`mario at esempio dot it` resta riconosciuto), perché chi maschera un indirizzo lo maschera tutto;<br>• **civico attaccato al tipo di via**: `43 Court`, `225 St`, `2 Circle` erano indirizzi. Ora serve almeno una parola in mezzo — il nome della strada, che in un indirizzo vero c'è sempre.<br><br>**Costo misurato: zero.** Richiamo invariato su 29 297 documenti (nomi 99,4%, email 99,9%, cards e CF 100%), e il corpus di conformità cambia in **un solo caso**, che era il difetto congelato come atteso | **Fatta** (1.21.0) |

---

## P8 — Per la 1.25.0 (2026-08-11)

**Da dove vengono.** Non da un'analisi. Due le ha trovate il port TypeScript
mentre si allineava al corpus della 1.24.0 — cioè un secondo motore che legge
le stesse regole e nota quello che il primo dà per scontato — e una la
ricostruzione del pacchetto, inciampandoci.

| ID | Item | Perché, e cosa costa | Stato |
|----|------|--------|--------|
| P8.1 | **Anteprima prima/dopo anche per i `.docx`** | Il PDF ce l'ha dalla 1.23.0, ed è ciò che permette a chi deposita un atto di **vedere** cosa è sparito prima di consegnarlo. Per Word non c'era, e il `.docx` è il formato in cui si scrivono le lettere.<br><br>**La forma non è la stessa, e lo dice a schermo.** Un PDF ha pagine; un `.docx` no, finché Word non lo impagina, e Word non è una dipendenza. Due colonne di contenuto (mammoth, già nel pacchetto). Senza la riga «non è l'impaginazione» la funzione inganna. | **Fatta** (1.25.0) |
| P8.2 | **Età e sesso: quattro forme che non vengono rilevate** | I gruppi `(?i:…)` coprivano metà della parola: `45 anni d'ETÀ`, `sesso: f`, `d' anni 78`, `Eta': 45` non finivano nel rapporto. Nessun dato usciva — questi riconoscitori non sostituiscono. Il danno era il verbale che mentiva. | **Fatta** (1.25.0) |
| P8.3 | **Il cancello blocca la build che lo farebbe passare** | `mancanti()` sul `dist/` vero stava nel quality gate, prima che la build ricostruisse la cartella. Un pacchetto a metà rendeva rosso il gate e impediva la riparazione. Il controllo è rimasto, e può ancora far fallire il build: si fa dopo la copia, `make_installer.py --controlla`. | **Fatta** (1.25.0) |
| P8.4 | **Build per Linux e macOS** | Oggi Mr. Rao si installa solo su Windows, e il motore non ha niente di specifico di Windows: lo sono il tray, la scorciatoia degli appunti, la finestra e il rilevamento del pacchetto MSIX. Le proposte stanno in `Builds/` (cartella non tracciata), scritte il 2026-08-11.<br><br>**Il passo 0 non è costruire: è far girare i test su Ubuntu.** Ci sono almeno quattro punti che oggi fuori da Windows non reggono, il principale in `mr_rao/appunti.py`, dove `ctypes.WinDLL` viene chiamato **senza guardia di piattaforma** su un percorso che la configurazione di serie raggiunge. Finché quello non è verde, parlare di PyInstaller e `.AppImage` è prematuro: si impacchetterebbe codice di cui non si sa se parte.<br><br>**E una decisione da prendere prima, non dopo**: su Linux e macOS non esistono né il menu contestuale del registro né lo Store, quindi «cosa vuol dire installato» va deciso, non ereditato da Windows | Da fare |

---

## P9 — Dai nomi che passavano (2026-08-14)

**Da dove vengono.** Da un documento vero: il cognome «Di Salvo» non veniva
anonimizzato. La causa — la particella dei cognomi composti è una
preposizione, quindi sta fra le parole comuni, quindi spezzava la sequenza —
è corretta. Guardando lì accanto ne sono emerse altre due, e questa è la
sezione che le tiene.

| ID | Item | Perché, e cosa costa | Stato |
|----|------|--------|--------|
| P9.1 | **Cognomi composti: «Walter Di Salvo» usciva intero** | La particella spezzava il tratto di parole maiuscole, e dopo un titolo usciva **mezzo nome** (`il sig. {{NAME_1}} Di Salvo`). Tre agganci — forma incollata negli elenchi, nome di battesimo davanti, parola nota dopo — più 85 composti aggiunti. | **Fatta** |
| P9.2 | **Il nome di battesimo da solo, quando non è anche una parola** | **FATTA, spenta di serie** (`privacy_names_alone`). Di serie un nome isolato resta un sospetto: «Rosa», «Vera», «Costa» sono nomi *e* parole. L'opzione tratta gli altri — 891 dei 1017 elencati — e a renderla utilizzabile è la guardia sul contesto: «ospedale Umberto», «via Umberto», «villa Ada», «stadio Giuseppe», «Sant'Antonio», «pizza Margherita» restano, perché nella parola non c'è niente che li distingua e a decidere è l'ultima parola prima (saltando articoli e preposizioni: in italiano fra l'edificio e il nome ce n'è sempre uno).<br><br>**Misurata prima di essere accolta**, com'era scritto qui: `scripts/bench_nomi_isolati.py` con 42 casi in due popolazioni opposte, gli stessi nomi di qua e di là — Vittorio, Umberto, Leonardo, Giuseppe. **+15 presi, 0 falsi positivi nuovi**, in prosa e su modulo.<br><br>Restano fuori i nomi che sembrano parole italiane (Vittorio `-orio`, Federica `-ica`): il veto morfologico c'era già ed è quello che tiene la regola lontana dai guai. E resta fuori il **cognome** isolato, che non dice se sia una persona o l'azienda che porta quel nome.<br><br>Casella nella pagina, corpus di conformità con l'interruttore nei due versi, port nell'estensione. | **Fatta** (spenta di serie) |
| P9.3 | **Il saluto che annuncia un nome** | «Ciao Pietro», «Caro Marco», «Gentile Anna»: una formula di apertura dichiara che quello che segue è una persona, esattamente come fa una formula di chiusura in `_RE_FIRMA_IT` — che esiste già ed è il posto dove un cognome da solo è davvero un cognome.<br><br>È il pezzo di P9.2 che si può prendere **senza** la misura, perché non allarga il caso isolato: aggiunge un contesto, come il titolo professionale e l'indirizzo di posta. Prezzo basso, e prende il caso più frequente nelle email e nelle chat.<br><br>**Fatta**: `_RE_SALUTO_NOME`, con otto negativi nel banco («Gentile Cliente», «Ciao Team», «Salve Ufficio Protocollo») e cinque casi nel corpus di conformità | **Fatta** |
| P9.4 | **Le intitolazioni diventano persone**: «stadio Giuseppe Meazza», «premio Italo Calvino» | Il riconoscitore delle coppie non sa che quello è uno stadio: due parole maiuscole adiacenti, entrambe negli elenchi, e la sequenza sparisce. Lo scudo delle parole d'ente copre `teatro`, `istituto`, `ospedale` — ma solo quando la parola d'ente è **adiacente**, e `stadio`, `premio`, `ponte`, `villa`, `torneo` in quell'elenco non ci sono affatto.<br><br>**Misurato il 2026-08-14 sul corpus pubblico**, non ipotizzato: su undici Gazzette Ufficiali il motore fa 475 sostituzioni di nome, e fra quelle ci sono le intitolazioni. Sui casi costruiti: «Si gioca allo stadio Giuseppe Meazza» → `{{NAME_1}}`, «Ha vinto il premio Italo Calvino» → `{{NAME_1}}`, «Basilica di San Marco» → `{{NAME_1}}` in prosa.<br><br>**Non è la stessa cosa di P9.2**, ed è il motivo per cui è una voce a sé: là il rischio era il nome **isolato** e si è chiuso con la guardia sul contesto (quella guardia ha già `stadio`, `premio`, `villa`); qui il problema è la **coppia**, che passa da un'altra strada e ha una soglia sua. La guardia di P9.2 non la tocca.<br><br>**Cosa costa sbagliare, nelle due direzioni.** Sostituire toglie il soggetto alla frase — «si gioca allo `{{NAME_1}}`» non è più un documento. Non sostituire lascia in chiaro il nome di una persona vera ogni volta che un'intitolazione somiglia a un nome, e sulle Gazzette quelle persone ci sono davvero.<br><br>**Da fare con la misura, come P9.2**: estendere l'elenco delle parole d'ente non basta e va provato — `_ENTITY_WORDS` scherma **l'intera** sequenza, quindi ogni parola aggiunta è una persona che smette di essere protetta quando quella parola le sta davanti. Il commento sopra `_ENTITY_WORDS` lo dice già, e va riletto prima di toccarlo. **Ed e' andata cosi': l'elenco non e' stato allungato** | **FATTA**, non ancora rilasciata. La guardia non tocca `_ENTITY_WORDS`: legge **la coda di lettere e spazi davanti alla sequenza**, saltando articoli, altre maiuscole della stessa intitolazione e gli aggettivi che qualificano l'edificio, e si ferma a **qualunque segno di punteggiatura** — cosi' «Residenza: Mario Rossi» e «Zona 3 - referente Mario Rossi» restano protetti, e «il premio e' stato consegnato a Mario Rossi» pure, perche' un verbo chiude la risalita. In piu' la parola che decide viene guardata quando sta **in testa** alla sequenza («San Giovanni Rotondo», «Sant'Antonio Abate»), e solo in testa: «Mario Villa» resta protetto.<br><br>**Misura**: falsi positivi di `bench_nomi_isolati` da 14 a 0 in prosa e da 9 a 0 su modulo; sul corpus pubblico i moduli in bianco **scendono** (da 3 nomi a 1) e la prosa vera non perde niente. Sette casi nuovi nel corpus di conformita', due dei quali contrappesi. Prezzo dichiarato e congelato da un test: «presso casa Mario Rossi» non viene piu' sostituito.<br><br>**Trovati strada facendo, e sono la parte piu' utile**: `bench_nomi_isolati` non poteva piu' fallire (confrontava le opzioni di serie con `names_alone=True`, che dalla 1.26.0 sono la stessa cosa), e `Virginia` sui moduli IRS scampava per caso, perche' la guardia dei nomi soli risaliva all'indietro senza badare alle virgole |

---

## Ordine di lavoro consigliato

**Riscritto il 2026-08-11.** La 1.19.0 aveva chiuso P0.3, P3.17 e P4.7, cioè
le tre voci che stavano in cima; **non c'è più nessuna P0 aperta**, per la
prima volta da quando questo elenco esiste. Il 2026-08-11 è arrivata la
certificazione dello Store, che chiude P3.9 e toglie dalla catena la sua
attesa.
...
**P3.9 è chiusa il 2026-08-11.** L'app è pubblicata sullo Store, l'indirizzo
non risponde più `410`, e il link è entrato nei due README. Non è più una
voce che si controlla: è fatta.

**P3.6 è chiusa, e non sta più nella catena.** Questa riga diceva
«approvato e sbloccato», ed era rimasta indietro di un giorno rispetto alla
propria scheda: la decisione «si fa» del 2026-08-09 è stata ribaltata dalla
misura fatta lo stesso giorno — il NER non ha battuto quello che il motore già
fa sui nomi, e nel cancello difendibile il guadagno è **zero**. La parte utile
di quell'indagine è diventata **P3.18**, fatta nella 1.17.0. Riaprirla richiede
un guadagno misurato, non una nuova opinione.

**P3.10 sale in cima.** La prima pubblicazione sullo Store è stata manuale
perché l'automazione aggiorna e non inserisce; ora che c'è un'app da
aggiornare, il presupposto che teneva ferma quella voce non c'è più.
